# 远程控制工具 更新日志（AI生成仅供参考）

# V2.3.3 (2025-09-13)

**开机自启稳定性修复**  
**Full Changelog**: https://github.com/chen6019/Remote-Controls/compare/V2.3.2...V2.3.3

## 🛡 关键稳定性修复
- **修复开机自启后长时间空闲崩溃问题**：针对用户反馈的"开机进入桌面很长时间后点击托盘状态出现 Error launching CrashSender.exe 报错"进行专项修复。
- **长时间空闲检测**：新增启动时间记录机制，自动检测开机后超过30分钟的长时间空闲状态。
- **增强重试策略**：
  - 长时间空闲后首次调用：`ShellExecuteW` 重试3次，间隔0.5秒
  - 正常使用场景：保持1次调用，确保响应速度
  - 首次调用前增加0.2秒延迟，让系统组件充分初始化

## 🔧 系统库调用优化
- **避免句柄缓存问题**：每次调用重新获取 `ctypes.windll.shell32` 句柄，防止长时间缓存导致的状态异常。
- **回退机制增强**：`webbrowser.open` 也加入2次重试机制，提升整体成功率。
- **错误提示优化**：区分不同失败原因，提供更具体的用户提示信息。

## 📊 诊断与日志
- **详细运行状态记录**：启动时记录时间戳，便于分析长时间运行场景。
- **分层日志记录**：区分长时间空闲和正常调用的处理路径，便于问题追踪。
- **重试过程透明化**：详细记录每次重试的结果和错误原因。

## 🎯 问题解决效果
- **根本解决**：开机自启托盘程序在长时间空闲后不再出现 CrashSender.exe 报错和闪退。
- **保持兼容**：正常使用场景下的响应速度和行为完全不变。
- **预防性处理**：针对各种系统环境差异和资源加载时序问题提供容错。

## 📋 测试建议
建议用户在以下场景验证修复效果：
1. 开机自启后等待30分钟以上再点击"托盘状态"
2. 系统从休眠/睡眠状态唤醒后的首次点击
3. 资源紧张或启动缓慢的系统环境

> 本版本专注解决稳定性问题，功能行为与 V2.3.2 保持一致。
