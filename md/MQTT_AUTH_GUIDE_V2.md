...
配置入口：推荐使用 `RC-GUI.exe` 配置并保存，程序会生成/更新同目录的 `config.toml`。
...
-   **配置方式**：
    ...
    -   客户端 ID：填写私钥（会写入 `config.toml` 的 `client_id`）
    -   端口：通常为 9501（巴法云）
...
1. **连接失败**：
    ...
    - 检查网络连接是否正常
    - 确认服务器地址和端口正确（项目默认已链接 Paho SSL 库 `paho-mqtt3cs`，支持 TLS）
    - 验证认证信息是否正确
...
5. **TLS/SSL 支持**：
    - 主程序必须链接 `paho-mqtt3cs` 或 `paho-mqtt3as` 库才能支持 TLS
    - 若构建时未包含 SSL 支持，设置 `mqtt_tls = 1` 会导致连接失败或程序报错
    - `mqtt_tls_ca_file` 建议使用绝对路径，若使用相对路径则是相对于主程序所在的目录
