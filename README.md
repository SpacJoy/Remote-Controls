...
[Contributing](CONTRIBUTING.md) • [Changelog](C近期新增 / 强化（2.2.8 → 2.3.4）：
- **V2.3.4 (2025-09-15)**: 异步MQTT连接与自动重连，弱网环境稳定性大幅提升；修复paho-mqtt v2兼容性问题；增强消息处理健壮性，避免单条消息异常中断进程。
- **V2.3.2 (2025-09-03)**: 全局异常捕获避免CrashSender弹窗；托盘异步打开浏览器并增加节流；移除本地彩蛋图片打包，减小体积；统一浏览器调用逻辑，提升兼容性。
...
- 构建清洁性：打包过程不修改源文件，临时文件自动清理.md) • [Full history](md/up.md)
...
— 当前文档对应版本：2.2.13     
（版本号统一由 CI/本地 打包自动注入；完整更新历史见 `md/up.md`）
...
```
- Q: MQTT 无法连接？
  A: 检查服务器地址、端口、认证模式与凭据是否正确，网络是否畅通。从V2.3.4起，程序支持弱网环境下的自动重连，无需手动重启。
- Q: 弱网或网络不稳定时程序会退出吗？
  A: 从V2.3.4起，程序改为异步MQTT连接并启用自动重连（2s-30s退避），弱网场景下会持续重试并显示"正在重试"通知，网络恢复后自动清除提示，不会退出。
- Q: 托盘程序无法检测到主程序？
...
- **V2.3.4**: 更新弱网环境兼容性与启动稳定性，修复多项关键问题
- 修复托盘无法启动主程序的问题
- 添加网络重连机制，优化MQTT连接失败时的用户提示与重试逻辑
- 更新随机图片接口URL
- 增强管理员权限运行程序和脚本的安全性，添加系统就绪检查与多种启动方案
- 增强系统就绪检查与安全打开URL逻辑，解决计划任务启动时的崩溃问题

完整提交历史请查看 [GitHub Commits](https://github.com/chen6019/Remote-Controls/commits/main)。
